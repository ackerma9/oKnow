...
namespace OKnow.Questions
{
    public class MovieQuestions
    {
        public static void addQuestions(QuestionPool pool)
        {
            ...
            pool.addQuestion(Category.MUSIC, "In the Dirty Harry movies starring Clint Eastwood as Dirty Harry, what was Harry's last name?", choices, Answer.ONE);

            choices = new string[] { "$16 million", "$1 million", "$6 million", "$25 million" };
            pool.addQuestion(Category.MUSIC, "How much of his own money did Francis Ford Coppola put up to finish the movie  'Apocalypse Now' when it ran wildly over budget?", choices, Answer.ONE);

            choices = new string[] { "Full Metal Jacket", "Fatal Attraction", "Broadcast News", "Angel Heart" };
            pool.addQuestion(Category.MUSIC, "What  1987 film was based on a novel called The Short Timers by Gustav Hasford?", choices, Answer.ONE);

            choices = new string[] { "Humphrey Bogart", "James Cagney", "Clint Eastwood", "Burt Lancaster" };
            pool.addQuestion(Category.MUSIC, "Which of the following actors has the middle name 'DeForest'?", choices, Answer.ONE);

            choices = new string[] { "Skull Island", "Monster Island", "Borneo", "Ape Island" };
            pool.addQuestion(Category.MUSIC, "What was the name of the island on which King Kong was discovered in the original 1933 movie?", choices, Answer.ONE);
            ...
            pool.addQuestion(Category.MUSIC, "In the movie Who Framed Roger Rabbit, which pair of genetically similar characters perform a piano duet?", choices, Answer.ONE);

            choices = new string[] { "$297,000", "$497,000", "$97,000", "$797,000" };
            pool.addQuestion(Category.MUSIC, "The Moon of Barods, a diamond that Marilyn Monroe wore when singing 'Diamonds Are A Girl's Best Friend' in the film Gentlemen prefer Blondes, was auctioned off at Christies for how much in 1990?", choices, Answer.ONE);

            choices = new string[] { "best actor", "best actress", "best picture", "best supporting actor" };
            pool.addQuestion(Category.MUSIC, "Which one of these Academy Awards did Gone With the Wind not win?", choices, Answer.ONE);

            choices = new string[] { "Cary Grant", "W.C. Fields", "James Stewart", "John Wayne" };
            pool.addQuestion(Category.MUSIC, "n the 1933 movie where May West spoke the line 'Come up and see me sometime' , called She Done Him Wrong, who was her co-star?", choices, Answer.ONE);

            choices = new string[] { "Dirty Harry", "Magnum Force", "Sudden Impact", "Tightrope" };
            pool.addQuestion(Category.MUSIC, "Clint Eastwood gave us the immortal line, 'Go ahead... make my day', in what film?", choices, Answer.ONE);
            ...
            pool.addQuestion(Category.TVSHOWS, "Why in the movie Edward Scissorhands did Edward never get real hands?", choices, Answer.ONE);

            choices = new string[] { "The magic man", "baked alone", "cal the stallion", "Shake N.." };
            pool.addQuestion(Category.TVSHOWS, "In the movie Talladega Nights: The Ballad of Ricky Bobby, what is Cal's new nickname after dumping Ricky?", choices, Answer.ONE);

            choices = new string[] { "The Day After Tomorrow", "Twister", "Volcano", "An Inconvient Truth" };
            pool.addQuestion(Category.TVSHOWS, "Which environmental awareness movie features Al Gore?", choices, Answer.FOUR);

            choices = new string[] { "Date Movie", "Scary movie 2", "Scary movie 3", "Scary movie 4" };
            pool.addQuestion(Category.TVSHOWS, "In what movie do they spoof the village, war of the worlds, and the grudge?", choices, Answer.FOUR);

            choices = new string[] { "A plane", "A man in a bunny suit", "An Asteroid", "A Jet Engine" };
            pool.addQuestion(Category.TVSHOWS, "In the movie, Donnie Darko, what falls from the sky into Donnie's Bedroom?", choices, Answer.FOUR);

            choices = new string[] { "Plasma Contamination", "Engine Imbalance", "Sabotage", "It was a wormhole" };
            pool.addQuestion(Category.TVSHOWS, "What is the cause for sending the Enterprise into a wormhole?", choices, Answer.TWO);

            choices = new string[] { "Mike Myres", "Freddy Kruger", "Jason Vorhees", "Jigsaw" };
            pool.addQuestion(Category.TVSHOWS, "What is the serial killer's name in the Friday the 13th horror movies?", choices, Answer.TWO);

            choices = new string[] { "Cameron Diaz", "Julia Roberts", "Eliza Dushku", "Kristen Dunst" };
            pool.addQuestion(Category.TVSHOWS, "Who plays Torrance Shipman in Bring it on?", choices, Answer.FOUR);

            choices = new string[] { "Bill Murray", "Dan Aykroyd", "Chevy Chase", "Kevin Nealson" };
            pool.addQuestion(Category.TVSHOWS, "Which Saturday Night Live veteran starred in Memoirs of an Invisble Man (1992)?", choices, Answer.THREE);

            choices = new string[] { "Reuben Studdard", "Kelly Clarkson", "Kerry Underwood", "Taylor Hicks" };
            pool.addQuestion(Category.TVSHOWS, "What American Idol winner made a special appearance in Scooby Doo 2?", choices, Answer.FOUR);
        }
    }
}
